					...
					return (
						<div key={index} className='py-4  border-t text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4'>
							<div className='flex items-start gap-6'>
								...
								<div>
									<div className='text-xs sm:text-lg font-medium'>{productData.name}</div>
							...
							<input type='number' min={1} defaultValue={item.quantity} className='border max-w-20 px-1 sm:px-2 py-1' onChange={(e) => (e.target.value === '' || e.target.value === '0' ? null : updateQuantity(item._id, item.size, Number(e.target.value)))} />
